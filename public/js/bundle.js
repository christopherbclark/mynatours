...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    ...
    //scrollZoom: 'false',
    showZoom: 'true' // center: [-80.743814,34.925885],
    // zoom: 15
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // Add a pop up

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); //Extend the bounds to include the current location
...
var showAlert = function showAlert(type, msg) {
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</type>");
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              console.log(type);
              (0, _alerts.showAlert)('success', "".concat(type.toUpperCase(), " updated successfully, BITCH!"));
              window.setTimeout(function () {
                location.assign('/me');
              }, 500);
            }
